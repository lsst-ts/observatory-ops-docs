...
4.  To move the MTRotator, select :guilabel:`Enabled sub-state command` 
    in the ``Command`` section. 
    
    a. Under ``Command Parameters``, navigate to ``Enabled sub-state trigger`` 
...
5.  If the MTRotator does not follow:
    ...
    a.  Transition to ``Standby`` state (step 3) followed by the ``Enabled`` state again to reset the internal 
        calculation of Simulink model. Then, do the p2p movement (step 4) to origin again.
