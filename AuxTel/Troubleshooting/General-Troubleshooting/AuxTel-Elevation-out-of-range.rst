...
This procedure should be used when the AuxTel :file:`auxtel/correct_pointing.py` script fails with the error *Rejected: elevation out-of-range*.
...
- The :file:`correct_pointing` script fails with the following error traceback:
...
- This occurs when the target search to run the correct_pointing script is not successful in the specified location in the sky within the `mag_limit`, `mag_range`, and `radius`.
...
To resolve the *Rejected: elevation out-of-range* error in the :file:`correct_pointing` script, follow these steps:

#. Try widening the target search. Run :file:`correct_pointing` with a configuration that represents a larger search area. 
    The script defaults are:

     .. code-block:: text
        :caption: :file:`correct_pointing` default configuration
    ...
    You can adjust the parameters to:
    
    - Decrease the `mag_limit` towards brighter sources.
