...
Starts the CCS shell from the bash prompt at *auxtel-hcu01.cp.lsst.org*. Then:

.. promt:: bash

   ccs-shell &

#. Set the ats subsystem to the highest level:

   .. code-block:: bash

      ccs> set level ats 10
...
#. The *ats-mcm* (which stands for Master Control Module) cannot be cleared in this way.  
...
#. Assuming the *SummaryState* is ``OFFLINE``, then we look at the *OfflineState*.  
    ...
    .. code-block:: bash
      ...
      ccs> ats-ocs-bridge setAvailable
